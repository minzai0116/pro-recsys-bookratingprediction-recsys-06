...
이 프로젝트에서는 단순한 `user-item` 상호작용만 보는 대신, 저자, 출판사, 카테고리, 지역, 나이와 같은 메타데이터를 함께 활용해 평점 예측 성능을 끌어올리는 데 집중했습니다. 특히 데이터 희소성과 범주형 피처 비중이 매우 높은 환경에서 어떤 모델이 실제로 강한지를 검증하고, 전처리와 피처 엔지니어링이 성능에 어떤 영향을 주는지 체계적으로 실험했습니다.

### 1.2. Project Objective
- 도서 평점 예측 문제에 적합한 모델 구조 탐색
- 범주형 중심 데이터에서의 Feature Engineering 효과 검증
- 위치 정보 정제, 평점 개수 기반 통계 피처 등 실전형 전처리 파이프라인 구축
- CatBoost, LightGBM, FM 계열, BERT 기반 모델을 비교해 최종 전략 도출

### 1.3. 내가 담당한 역할
- EDA를 통한 데이터 특성 분석
- CatBoost baseline 전처리 파이프라인 구축
- `Category`, `Location` 관련 Feature Engineering 설계 및 반영

## 2. Approach

### 2.1. 핵심 문제 정의
- 상호작용 데이터가 매우 희소하고, 대부분의 상호작용이 소수 엔티티에 집중됨
- `user`, `book`, `author`, `location`, `category` 등 범주형 피처 비중이 높음
- 단순 딥러닝 모델보다, 범주형 변수와 상호작용을 잘 처리하는 모델이 유리할 가능성이 큼
...
- 희소한 범주형 데이터에서는 FM 계열보다 GBDT 계열이 더 안정적이었습니다.
...
- 유저/도서/저자별 평점 개수 피처 추가 후 CatBoost 기준 `RMSE 2.474 -> 2.1318` 수준의 유의미한 개선이 있었습니다.
- 위치 정제 파이프라인 적용 후 GeoMap 기준 유효 데이터 비율이 `City 86.5% -> 89.1%`, `State 83.9% -> 97.3%`, `Country 95.7% -> 99.9%`로 향상됐습니다.

### 3.2. 최종 선택 전략
- 최종 모델은 `CatBoost` 기반 접근입니다.
- 이유는 범주형 피처 수가 많고, 각 범주의 cardinality가 큰 환경에서 안정적으로 동작했기 때문입니다.
- 모델 복잡도를 늘리는 것보다, 데이터 정제와 통계 기반 피처 추가가 더 큰 성능 개선을 가져왔습니다.
