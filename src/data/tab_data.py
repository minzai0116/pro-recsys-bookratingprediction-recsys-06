...
def remain_train_features_only(rating):
    ...
    특이사항 :   summary_index는 맨 앞에,
                summary index == tab에서 summary_vector 탐색용 index
    ...
    rating.insert(0, "summary_index", summary_index)
    rating.insert(1, "rating", scores)
    
    # 맨 처음이 summary_index
    # df_offset = [summary_index, rating, user_id, country, state, city.....]
    return (rating, col_len_set)
...
def text_to_vector(books, args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.model_args.tab_rec.pretrained_model)
    model = AutoModel.from_pretrained(args.model_args.tab_rec.pretrained_model).to(device=args.device)
    model.eval()
    ...
    for title, summary, book_id in tqdm(zip(books_['book_title'], books_['summary'], books_['summary_index']), total=len(books_)):
        ...
        with torch.no_grad():
            ...
            ### tab 모델의 경우, 최종 출력물의 사이즈가 (토큰길이, 임베딩=768)이므로, 이를 평균내어 사용하거나 pooler_output을 사용하여 [CLS] 토큰의 임베딩만 사용
...
def tab_data_load(args):
    ...
    if not args.model_args.tab_rec.prepared_summary:
    ...
    train_df, cardinality = remain_train_features_only(train_df)
    test_df, _ = remain_train_features_only(test_df)
