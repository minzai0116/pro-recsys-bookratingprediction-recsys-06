...
def run_catboost(args, data, setting):
    ...
    if args.dataset.valid_ratio != 0:
        X_val = data['X_valid'][all_features]
        y_val = data['y_valid']
        eval_set = [(X_val, y_val)]
    else:
        eval_set = None

    X_test = data['test'][all_features]
    ...
    if args.use_summary:
        ...
        print("\n>>> Summary Embedding Importance Check")
        emb_importance = fi_df[fi_df['feature'].str.contains('summary_pca')]
        print(f"임베딩 피처 개수: {len(emb_importance)}")
        print(f"임베딩 최대 importance: {emb_importance['importance'].max() if len(emb_importance) > 0 else 0}")
        print(f"임베딩 평균 importance: {emb_importance['importance'].mean() if len(emb_importance) > 0 else 0}")
        print("\n임베딩 Top 5:")
        print(emb_importance.head(5))
